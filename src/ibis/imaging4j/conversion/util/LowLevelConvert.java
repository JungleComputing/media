...
public class LowLevelConvert {
    ...
    public static final void RGB24toARGB32(int width, int height,
            ByteBuffer in, int[] rgbOut) {
    ...
    public static void YUYVtoARGB32(int width, int height, byte[] dataIn,
            int[] dataOut) {
    ...
    public static void YUYVtoARGB32Table2(int width, int height, ByteBuffer in,
            ByteBuffer out) {
        ...
        for (int i = 0; i < width * height * 2; i += 4) {
            ...
            int Y1 = (0xff & in.get(i + 0));
            int U = (0xff & in.get(i + 1));
            int Y2 = (0xff & in.get(i + 2));
            int V = (0xff & in.get(i + 3));

            out.put(2 * i + 0, (byte) 0xff);
            out.put(2 * i + 1, (byte) clipAndScale(Y1 + V1[V]));
            out.put(2 * i + 2, (byte) clipAndScale(Y1 - V2[V] - U1[U]));
            out.put(2 * i + 3, (byte) clipAndScale(Y1 + U2[U]));

            out.put(2 * i + 4, (byte) 0xff);
            out.put(2 * i + 5, (byte) clipAndScale(Y2 + V1[V]));
            out.put(2 * i + 6, (byte) clipAndScale(Y2 - V2[V] - U1[U]));
            out.put(2 * i + 7, (byte) clipAndScale(Y2 + U2[U]));
        }
    }
    ...
    public static void YUYVtoARGB32(int width, int height, ByteBuffer in,
        ...
        // System.out.println("Converting: " + in.capacity() + " -> " +
        // out.capacity());
        ...
        for (int h = 0; h < height; h++) {
            for (int w = 0; w < width * 2; w += 4) {
                ...
                tmpOut[2] = (byte) clipAndScale(Y1 - (0.698001 * V)
                        - (0.337633 * U));
                tmpOut[3] = (byte) clipAndScale(Y1 + +(1.732446 * U));

                tmpOut[5] = (byte) clipAndScale(Y2 + (1.370705 * V));
                tmpOut[6] = (byte) clipAndScale(Y2 - (0.698001 * V)
                        - (0.337633 * U));
                tmpOut[7] = (byte) clipAndScale(Y2 + (1.732446 * U));

                /*
                 * final int Y1 = (0xff & tmpIn[0]); final int U = (0xff &
                 * tmpIn[1]); final int Y2 = (0xff & tmpIn[2]); final int V =
                 * (0xff & tmpIn[3]);
                 * 
                 * tmpOut[1] = clipAndScale((298 * Y1 + 409 * U + 128) >> 8);
                 * tmpOut[5] = clipAndScale((298 * Y2 + 409 * U + 128) >> 8);
                 * 
                 * tmpOut[2] = clipAndScale((298 * Y1 - 100 * V - 208 * U + 128)
                 * >> 8); tmpOut[6] = clipAndScale((298 * Y2 - 100 * V - 208 * U
                 * + 128) >> 8);
                 * 
    ...
    public static void YUV420SPtoRGB24(int width, int height, byte[] dataIn,
            byte[] dataOut) {
    ...
    public static void YUV420SPtoRGB24(int width, int height, ByteBuffer in,
            byte[] dataOut) {
    ...
    public static void YUV420SPtoARGB32(int width, int height, ByteBuffer in,
            int[] dataOut) {
        ...
        for (int h = 0; h < height; h += 2) {
            for (int w = 0; w < width; w += 2) {
                ...
                dataOut[index++] = 0xFF000000 & ((byte) (0xff & r1)) << 16
                        & ((byte) (0xff & g1)) << 8 & ((byte) (0xff & b1));

                dataOut[index++] = 0xFF000000 & ((byte) (0xff & r2)) << 16
                        & ((byte) (0xff & g2)) << 8 & ((byte) (0xff & b2));

                dataOut[index++] = 0xFF000000 & ((byte) (0xff & r3)) << 16
                        & ((byte) (0xff & g3)) << 8 & ((byte) (0xff & b3));

                dataOut[index++] = 0xFF000000 & ((byte) (0xff & r4)) << 16
    ...
    public static void YUV422SPtoRGB24(int width, int height, byte[] dataIn,
            byte[] dataOut) {
    ...
    public static void YUV422SPtoRGB24(int width, int height, ByteBuffer in,
            byte[] dataOut) {
    ...
    public static void YUV422SPtoARGB32(int width, int height, ByteBuffer in,
            int[] dataOut) {
    ...
    public static void YUYVtoRGB24Table(int width, int height, ByteBuffer in,
        ...
        final byte[] tmpIn = new byte[2 * width];
        final byte[] tmpOut = new byte[3 * width];
        ...
        for (int h = 0; h < height; h++) {
            ...
            for (int w = 0; w < 2 * width; w += 4) {
                ...
                tmpOut[2 * w + 0] = (byte) clipAndScale(Y1 + V1[V]);
                tmpOut[2 * w + 1] = (byte) clipAndScale(Y1 - V2[V] - U1[U]);
                tmpOut[2 * w + 2] = (byte) clipAndScale(Y1 + U2[U]);

                tmpOut[2 * w + 3] = (byte) clipAndScale(Y2 + V1[V]);
                tmpOut[2 * w + 4] = (byte) clipAndScale(Y2 - V2[V] - U1[U]);
                tmpOut[2 * w + 5] = (byte) clipAndScale(Y2 + U2[U]);
            }
